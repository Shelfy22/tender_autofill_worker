...
10. Детерминированное и LLM-извлечение товарных позиций и количества.
...
productQuery
  ...
  -> LLM выбирает полное соответствие, допустимый аналог или "Товар не найден"
```
...
Важно: приложенный n8n export не раскрывает payload schema коллекции и содержит отсоединённую Qdrant-цепочку. Текущий Python передаёт найденные `id`, `score` и полный `payload` в LLM без угадывания названий полей. Перед переключением production нужен shadow test на реальных позициях.
