...
которые явно относятся к позиции, верни также в products[].characteristics отдельными
парами name/value с коротким evidence. Не включай туда количество закупки, цену, адрес,
срок/условия поставки, гарантию, оплату и требования к документам.
...
Ячейки характеристик добавляй в characteristics/requirements, а адреса, коды, цены и
условия поставки — только в evidence/requirements; не создавай из них отдельные products.
Для каждой позиции из Excel заполни sourceReference: fileName, sheet, row,
productColumn, quantityColumn, unitColumn, sectionRole и extractionMethod="llm". Не придумывай
координаты: если их нет в тексте, оставь соответствующие поля пустыми/null.
sectionRole используй из: technical_specification для ТЗ/технических требований,
price_justification для обоснования/расчёта НМЦ и сведений о цене каждой единицы,
contract для проекта договора, other если роль определить нельзя.
...
1. products — закупаемые товарные позиции из текста. Для каждой позиции сохрани связанные
   с ней технические характеристики в products[].characteristics как отдельные name/value.
   Сохрани lotNumber, positionNumber, model и article, когда они явно указаны. Не выдумывай их.
2. characteristicSets — характеристики из отдельной таблицы, раздела или файла, которые
   нельзя безопасно вложить в products прямо сейчас. Обязательно укажи productHint и, если
   есть, brand/article/model, lotNumber и positionNumber, чтобы Consolidator мог сопоставить их.
3. reasonHits — только реально подтверждённые недетерминированные причины отказа. Не возвращай false для остальных причин.
4. fieldCandidates — найденные поля карточки тендера, если они явно есть в этом unit.
...
- Характеристика должна описывать товар, а не тендер: не включай quantity закупки, цену,
  адрес, срок поставки, гарантию, оплату, документы, аналоги и общие условия договора.
- Связывай характеристики по номеру позиции, артикулу, модели, наименованию, строке/таблице
  и явным ссылкам между разделами. Не угадывай связь только по близости текста.
- Отдельный файл может целиком описывать один товар: используй имя файла, заголовок,
  модель/артикул и текст как доказательство productHint. Если связь неоднозначна,
  оставь набор в characteristicSets, не присоединяй его к случайному товару.
- Отсутствие characteristics допустимо и не означает ошибку анализа.
- positionKey входного products сохраняй без изменений. При сопоставлении отдельного набора
  укажи его targetPositionKey и associationConfidence 0..1; ниже 0.80 не присоединяй.
- Учитывай роль раздела. Обоснование цены обычно содержит полный перечень товаров,
  количества и цены, но может не содержать характеристик. ТЗ/технические требования
  содержат товары и их характеристики. Если одна позиция присутствует в обоих разделах,
  это один товар: сохрани позицию перечня и присоедини к ней характеристики из ТЗ.
- Один файл может быть составным и включать проект договора, ТЗ и обоснование цены.
  Не считай разделы такого файла одним источником дублей; заполняй sourceReference.sectionRole.
...
- проверить products[].characteristics и characteristicSets, сопоставить подтверждённые
  технические характеристики с нужными товарами, включая таблицу/файл характеристик,
  отдельные от файла со списком товаров;
- удалить из характеристик количество закупки, цены, адреса, сроки, гарантии, условия
  поставки/оплаты, требования к документам и другие факты, не описывающие SKU;
- не присоединять набор при неоднозначной связи; сохранить его в characteristicSets и warning;
- сохранять positionKey каждой позиции без изменения; targetPositionKey набора обязан
  ссылаться на существующий positionKey;
- считать одинаковую позицию из price_justification и technical_specification одним
  товаром: количество/цену брать из обоснования цены, характеристики переносить из ТЗ;
- одинаковые названия внутри одного раздела не объединять: сопоставлять повторяющиеся
  строки между разделами попарно в исходном порядке;
- отсутствие характеристик у товара допустимо: не удалять товар и не делать анализ incomplete;
...
Правила:
- Оставляй purchase_item только когда строка описывает физически поставляемый товар/материал/оборудование,
  который можно искать в каталоге поставщика. Наличие quantity само по себе не доказывает, что это товар.
- Если строка выглядит как условие под уже указанным товаром, классифицируй service или characteristic,
...
- Отдельные значения характеристик не являются товарами: "не менее 12 месяцев", "220 В", "IP54",
  "габариты", "материал", "цвет", "толщина", "мощность", "количество полюсов", "значение показателя".
- Адреса, даты, номера строк, ОКПД/КТРУ/ТН ВЭД, ИНН/КПП, телефоны, email, имена организаций,
  суммы и проценты без товарного наименования классифицируй как address/header/service/characteristic.
...
- characteristics должны описывать именно эту товарную позицию. Не считай отдельными
  товарами строки характеристик; сомнительная характеристика не является основанием
  удалить подтверждённый purchase_item.
...
""".strip()
    ...
    def classify_product_characteristics(
        ...
        prompt = f"""
Ты — семантический классификатор товарных характеристик для большого промышленного каталога.
Каталог содержит электротехнику, кабели, КИПиА, насосы, вентиляцию, инструмент, крепёж,
телекоммуникации, IT, масла, жидкости, материалы, сантехнику и складское оборудование.

Для каждого position_index:
1. Определи категорию товара.
2. Определи identifier_strength: HIGH — точная модель/серия/артикул; MEDIUM — серия или
   неполное обозначение; LOW — только общий тип товара.
3. Классифицируй каждую входную характеристику, сохраняя её id.
4. Определи value_form и компактный search_token.
5. Определи, уже закодирована ли характеристика в названии.

На этом этапе НЕ решай, добавлять ли характеристику в поиск.

semantic_role:
- IDENTIFIER: бренд, серия, модель, артикул, маркировка;
- VARIANT_SELECTOR: параметр конкретного исполнения;
- SIZE_OR_DIMENSION: размер, диаметр, длина, сечение, толщина, типоразмер;
- PACKAGING: фасовка, объём/масса упаковки;
- PERFORMANCE: мощность, производительность, подача, напор, скорость;
- INTERFACE_OR_STANDARD: сигнал, интерфейс, схема, стандарт, резьба;
- QUALITY_OR_PRECISION: точность, разрешение, погрешность, класс качества;
- ENVIRONMENT: IP, температура, климатическое исполнение, влажность;
- TEMPORAL: гарантия, срок службы, ресурс, наработка, срок эксплуатации;
- QUANTITY: количество закупаемых единиц;
- ENUMERATION: набор уставок, режимов или вариантов;
- OTHER: всё остальное.

value_form:
- EXACT: 220 В, 75 кВт, A3, IP65;
- ACCEPTABLE_RANGE: 25-32 кВт, 45-55 А;
- MINIMUM: не менее 200 м3/ч, >=600 dpi;
- MAXIMUM: не более 50 м, <=10 кг;
- TOLERANCE: 200±10 мм, 6,3±0,6 В;
- ENUMERATED: 2;4;8;16 или 100-75-50-25%;
- INTERFACE_RANGE: 4-20 мА, 0-10 В, 50/60 Гц;
- TEMPERATURE_RANGE: -40...+60 °C;
- TEXT: лазерная, цветная, медь.

search_token — короткое значение без длинного названия параметра: 220В, 2000А, 4-20мА,
A3, лазерная, IP65, M20x1,5. Если голое число неоднозначно, используй каноническую форму,
например «3P», а не «3». Для диапазонов заполни numeric.min/max, для допуска numeric.nominal,
для единиц numeric.unit. Не вычисляй midpoint сам: это сделает код.

already_encoded_in_name=true, если значение явно или семантически уже есть в исходном
названии: ПВС 3x0,75; ВА55-43 2000А; WD-40 200 мл; код модели, кодирующий диапазон.

Ничего не придумывай, не меняй модель товара и не заменяй класс товара другим.
Верни ровно один result для каждого position_index и ровно одну характеристику для каждого
переданного id. Используй строго JSON по схеме.
...
""".strip()
        return self.json_call(
            system=(
                "Классифицируй смысл и числовую форму характеристик товара. "
                "Не выбирай параметры для поиска. Только JSON."
            ),
            prompt=prompt,
            schema=ProductSemanticBatchResponse,
            operation="classify_product_characteristics",
        )

    def classify_sku_importance(
        self,
        items: list[dict[str, Any]],
    ) -> ProductSkuBatchResponse:
        prompt = f"""
Ты решаешь, какие характеристики действительно нужны для первичного поиска конкретного SKU
в большом промышленном каталоге. Тебе переданы категория, исходное название,
identifier_strength и нормализованные характеристики.

Идентифицирующий параметр — тот, изменение которого внутри одной товарной серии обычно
создаёт другой артикул, модификацию или исполнение. Для каждой характеристики спроси:
1. Изменится ли обычно SKU, если изменить только это значение?
2. Существенно ли параметр отличает правильный товар от похожих?

sku_importance:
- CRITICAL: без параметра велик риск выбрать другую модификацию/товар;
- HIGH: сильно различает варианты серии;
- MEDIUM: полезен для проверки, но обычно не нужен в первичном поиске;
- LOW: слабый идентификатор;
- NONE: не помогает определить товар.

usage:
- SEARCH_PRIMARY: критически важный идентификатор;
- SEARCH_SECONDARY: полезно добавить к поиску;
- VALIDATION_ONLY: только проверка кандидатов;
- IGNORE: не использовать.

Точный тип, бренд, серия, модель и артикул важнее отдельных чисел. Числовое совпадение
никогда не может изменить класс товара: привод != клапан, радиолампа != светодиодная лампа,
ПВС != другой кабель, холодильный компрессор != воздушный компрессор.

Учитывай категорию. Для электротехники часто важны ток, напряжение, полюса, отключающая
способность; для кабелей — марка, жилы, сечение; для КИП — диапазон, сигнал, присоединение;
для насосов — тип, модель, мощность, подача, напор, DN; для двигателей — мощность,
напряжение, обороты, типоразмер; для масел — бренд, серия, вязкость, фасовка; для труб —
материал, диаметр, стенка, присоединение; для крепежа — тип, диаметр, длина, резьба, класс.

При identifier_strength HIGH обычно выбери 0-1 параметр, MEDIUM — 1-3, LOW — 2-5.
Максимум 5, предпочтительно 0-3. already_encoded_in_name=true не выбирай.
TEMPORAL и QUANTITY всегда IGNORE. ENUMERATED и TEMPERATURE_RANGE обычно VALIDATION_ONLY.
ENVIRONMENT обычно VALIDATION_ONLY, кроме специального исполнения. QUALITY_OR_PRECISION
обычно VALIDATION_ONLY. PACKAGING может быть HIGH для жидкостей. SIZE_OR_DIMENSION может
быть CRITICAL для крепежа/труб/подшипников и LOW для габаритов корпуса.

Не создавай поисковую строку, не переписывай search_token и не меняй значения.
Выбирай только существующие id. Верни ровно один result для каждого position_index и одно
decision для каждого id. selected_for_search должен содержать только SEARCH_PRIMARY или
SEARCH_SECONDARY, не более пяти id. Только JSON по схеме.
...
""".strip()
        return self.json_call(
            system=(
                "Определи SKU-важность уже нормализованных характеристик. "
                "Не создавай поисковую строку. Только JSON."
            ),
            prompt=prompt,
            schema=ProductSkuBatchResponse,
            operation="classify_sku_importance",
        )
