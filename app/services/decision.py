...
REASONS = [
    ...
    "Коммерческие условия. Отсрочка платежа более 120 рабочих (180 календарных) дней",
...
def calculate_hard_reasons(
    ...
    patterns: list[tuple[str, str, int]] = [
        ...
        (r"\b(республик[а-я]*\s+саха(?:\s*\(якутия\))?|якут(?:ия|ск[а-я]*)|калининград(?:ск[а-я]*)?)\b",
         "Коммерческие условия. Поставка в удаленные территории", 40),
    ...
    for match in re.finditer(
        r"(?:отсрочк[а-я]*|оплат[а-я]*\s+в\s+течение)[^\d]{0,80}(\d+)\s*(рабоч|календарн)[а-я]*\s+д",
        all_text,
        re.I,
    ):
        count, day_type = int(match.group(1)), match.group(2).lower()
        if (day_type.startswith("рабоч") and count > 120) or (
            day_type.startswith("календарн") and count > 180
        ):
            _add(
                reasons,
                "Коммерческие условия. Отсрочка платежа более 120 рабочих (180 календарных) дней",
...
- Удалённые территории: Якутия/Саха и Калининград.
